...
export function publishStatus(snapshot) {
  ...
  const balances = cashBalances(ledgerResult.entries, {
    ...
    personal_opening_usd: Number(snapshot.personal_opening_usd ?? snapshot.pre_lab_cash_usd ?? PRE_LAB_PERSONAL_CASH_USD),
  });
  ...
  const v3 = computeLabHeadlineV3({
    ...
    pre_lab_cash_usd: Number(snapshot.personal_opening_usd ?? snapshot.pre_lab_cash_usd ?? PRE_LAB_PERSONAL_CASH_USD),
  });
