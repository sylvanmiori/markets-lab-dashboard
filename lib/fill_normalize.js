...
export function rawPriceCents(raw, side) {
  ...
  // Published combined side like buy_no @ 94 may actually be raw yes buy @ 6
  if (quoted != null && quoted !== side && pc >= 0 && pc <= 100) {
    return 100 - pc;
  }
...
export function resolveFromPublishedComplement(published) {
  ...
  const qty = toNum(published.qty);
  if (price == null || qty == null) return null;

  // Complement flip: buy_no@94 ↔ buy_yes@6
...
export function normalizeFill(raw, opts = {}) {
  ...
  const qty = toNum(raw.count ?? raw.qty ?? raw.quantity);
  const fee = toNum(raw.fee_cost ?? raw.fee_usd ?? raw.fees_usd);

  const fromRaw = resolveFromRawExchange(raw);
  let canonical = fromRaw;
  let confidence = fromRaw ? 'verified' : 'unknown';

  if (!canonical && opts.prefer_complement_correction !== false) {
  ...
  if (!canonical) {
    ...
    if (SIDE_ACTIONS.has(combined)) {
      const [action, side] = combined.split('_');
      const price = toNum(raw.price_cents);
  ...
  const feeUsd = fee != null ? -Math.abs(fee) : null;
...
export function normalizeRestingOrder(raw) {
  const qty = toNum(raw.count ?? raw.qty ?? raw.remaining_count);
    ...
    if (SIDE_ACTIONS.has(combined)) {
      ...
      if (price != null) {
        ...
        if (
          ...
          // Weather micro orders: high complement price with small declared risk → flip
          const inferred = resolveFromPublishedComplement(raw);
...
/**
 * FIFO realized P&L for normalized fills on one ticker.
...
export function matchRealizedPnl(fills) {
  ...
  for (const f of sorted) {
    ...
    const inv = inventoryDelta(action, side, f.qty);

    if (action === 'buy') {
      lots.push({ side, qty: f.qty, price_cents: f.price_cents, fee: f.fee_usd ?? 0 });
      results.push({ ...f, pnl_usd: null, pnl_kind: 'open_unrealized', pnl_status: 'open', pnl_label: 'open' });
      continue;
    }

    // sell — match against lots (same side)
    let remaining = f.qty;
    let pnl = f.fee_usd ?? 0;
