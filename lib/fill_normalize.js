...
export function signedFeeUsd(raw) {
  ...
  if (cost != null) return cost > 0 ? -cost : Math.abs(cost);
  if (raw.fee_usd != null) {
    const v = toNum(raw.fee_usd);
    if (v == null) return null;
    // Negative = paid (spec). Positive fee_usd without fee_cost is treated as paid magnitude.
    return v > 0 ? -v : v;
  }
  if (raw.fees_usd != null) {
    const v = toNum(raw.fees_usd);
    if (v == null) return null;
    return v > 0 ? -v : v;
  }
  return null;
